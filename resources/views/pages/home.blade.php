					...
					<div class="category-tab"><!--category-tab-->
						<div class="col-sm-12">
							<ul class="nav nav-tabs">
								...
								<li class="{{$key == 0 ? 'active':''}}"><a href="#{{$category->id}}" data-toggle="tab">{{$category->name}}</a></li>
								@endforeach
							</ul>
						</div>
						<div class="tab-content">
							@foreach($categories as $key => $category)
							    @php
                					$catproducts = App\Models\Product::where('category_id', $category->id)->get();
            					@endphp
								...
								<div class="item {{$key==0 ? 'active':''}}">
									...
									<div class="col-sm-4">
										<div class="product-image-wrapper">
											<div class="single-products">
												<div class="productinfo text-center">
													<img src="{{asset($prod->image)}}" alt="" width="268px" height="134px" />
													<h2>$56</h2>
													<p>Easy Polo Black Edition</p>
													<form action="{{route('add.cart', $prod->id)}}" method="POST">
														@csrf
														<button type="submit" class="btn btn-default add-to-cart"><i class="fa fa-shopping-cart"></i>Add to cart</button>
													</form>
												</div>
